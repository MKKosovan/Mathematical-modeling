...
Поскольку возникает коллинеарность между объектами, избавимся от Embarked

```{r, warning=FALSE}
model.qda <- qda(Survived ~ PassengerId + Pclass + Sex + Age + SibSp + Parch + Fare, data = titanic_train[inTrain, ])
model.qda
...
Возникает сильная чувствительность
...
plot(x1, y1, type = 'l', col = 'blue', lwd = 3,
     xlab = '(1 - SPC)', ylab = 'TPR',
     xlim = c(0, 1), ylim = c(0, 1), main = 'Training sample')
